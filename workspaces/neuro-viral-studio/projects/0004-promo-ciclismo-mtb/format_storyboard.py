...
def translate_vis(vis):
    ...
    if "llanta" in v or "rueda" in v: action_en = "close up of a mountain bike wheel"
    if "cadena" in v or "cassette" in v: action_en = "close up of bike chain and gears"
    if "dron" in v: action_en = "wide aerial shot of biker on a trail"
    if "salto" in v or "desnivel" in v: action_en = "biker jumping over the camera"
    if "derrapa" in v or "skid" in v: action_en = "biker skidding and kicking up dirt"
    if "logo" in v or "negra" in v: action_en = "abstract close up or solid color, no text"
