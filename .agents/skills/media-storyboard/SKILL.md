...
4. **Paginación y Prompts:** Agrupa las escenas en "bloques" de máximo 6 cuadros (Ej: Escenas 1-6, 7-12). Por cada bloque, **escribe explícitamente en el archivo markdown** un "Prompt Visual Global" genérico en inglés (en formato itálica). 
   - **FORMATO DEL PROMPT:** El prompt DEBE iniciar exigiendo explícitamente un grid tipo cómic. Por ejemplo: *"A 6-panel comic-style storyboard layout (2 rows of 3 panels)."* Luego debes describir brevemente qué pasa en cada panel (*"Panel 1: ... Panel 2: ..."*). Esto es crítico para que la IA grafique todo el storyboard y no solo una sola imagen resumen.
...
5. **Generación de Prueba (Test de Prompt):** Si tienes acceso a herramientas de generación de imágenes (como `generate_image`), **utiliza EXACTAMENTE EL PROMPT EN INGLÉS QUE ACABAS DE REDACTAR** para generar la imagen del bloque. Esto sirve para probar la calidad del prompt.
