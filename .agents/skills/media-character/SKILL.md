...
description: Registra un nuevo personaje en el Lore del workspace activo
---

# media-character

## When to use
Usa este comando cuando el usuario quiera registrar un nuevo personaje recurrente (actor, avatar, o elemento consistente como "La Salteña") para que sea utilizado en múltiples proyectos de video del mismo workspace.

## How to use
1. **Verifica el workspace activo:** Asegúrate de saber qué workspace está activo (ej. usando la configuración de `/media-in`).
...
5. **Crea la estructura:** Crea una carpeta en `workspaces/[WORKSPACE_ACTIVO]/characters/[ID_Tag]/`.
