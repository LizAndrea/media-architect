...
3. **Declaración de Reparto (CRÍTICO):** Verifica si el usuario pidió incluir personajes/props que existen en la carpeta `characters/` del workspace. Si es así, DEBES escribir al inicio del archivo la línea: `**Reparto (Lore):** [[ID_Tag1], [ID_Tag2]]`. Si no hay ninguno, escribe `**Reparto (Lore):** none`.
4. Genera un master script profesional utilizando la plantilla correspondiente en `templates/scripts/`. 
...
5. Analiza el "engagement score" y "viral potential" (usando rúbricas de `resources/metrics/`).
