...
4. Genera prompts ESTRUCTURADOS EN INGLÉS.
   ...
   - **USO DE TAGS (@) Y LORE GLOSSARY:** 
     - **Si el personaje ES visible:** Vuelve a inyectar el bloque `*LORE GLOSSARY:* \n *@ID_Rol = [Descripción]*` al inicio del archivo como referencia, y usa SIEMPRE el `@ID_Rol` dentro del prompt.
     - **Si el personaje NO ES visible (Ej: POV, screencast):** **OMITE POR COMPLETO** el bloque de Lore Glossary y no uses el tag. Si incluyes la descripción física de un personaje en un plano donde no debería estar, la IA alucinará e intentará forzar su aparición en pantalla.
